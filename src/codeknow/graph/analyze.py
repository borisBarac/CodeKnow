...
from codeknow.extract.detect import CODE_EXTENSIONS, IMAGE_EXTENSIONS, PAPER_EXTENSIONS
...
def _is_file_node(G: nx.Graph, node_id: str) -> bool:
    ...
    # (not just any label ending in .py)
    ...
    # These are real functions but structurally isolated by definition;
...
def suggest_questions(
            ...
            questions.append(
                {
                    "type": "ambiguous_edge",
                    "question": (
                        f"What is the exact relationship between `{ul}` and `{vl}`?"
                    ),
                    "why": (
                        f"Edge tagged AMBIGUOUS (relation: {relation})"
                        " - confidence is low."
                    ),
                }
            )
                ...
                questions.append(
                    {
                        "type": "bridge_node",
                        "question": (
                            f"Why does `{label}` connect"
                            f" `{comm_label}` to"
                            f" {', '.join(f'`{lbl}`' for lbl in other_labels)}?"
                        ),
                        "why": (
                            f"High betweenness centrality ({score:.3f})"
                            " - this node is a cross-community bridge."
                        ),
                    }
                )
    ...
    for node_id, _ in top_nodes:
        ...
        if len(inferred) >= 2:
            label = G.nodes[node_id].get("label", node_id)
            # Use _src/_tgt to get the correct direction;
            # fall back to v (the other node)
            others = []
            ...
            questions.append(
                {
                    ...
                    "question": (
                        f"Are the {len(inferred)} inferred relationships"
                        f" involving `{label}`"
                        f" (e.g. with `{others[0]}` and `{others[1]}`)"
                        " actually correct?"
                    ),
                    "why": (
                        f"`{label}` has {len(inferred)} INFERRED edges"
                        " - model-reasoned connections that need"
                        " verification."
                    ),
                }
            )
        ...
        questions.append(
            {
                ...
                "why": (
                    f"{len(isolated)} weakly-connected nodes found"
                    " - possible documentation gaps or missing edges."
                ),
            }
        )

    # 5. Low-cohesion communities → structural questions
    from codeknow.graph.cluster import cohesion_score
            ...
            questions.append(
                {
                    "type": "low_cohesion",
                    "question": (
                        f"Should `{label}` be split into smaller, more focused modules?"
                    ),
                    "why": (
                        f"Cohesion score {score}"
                        " - nodes in this community are"
        ...
        return [
            {
                ...
                "why": (
                    ...
                    "This usually means the corpus has no"
                    " AMBIGUOUS edges, no bridge nodes, "
                    "no INFERRED relationships, and all"
                    " communities are tightly cohesive. "
...
def graph_diff(G_old: nx.Graph, G_new: nx.Graph) -> dict:
    ...
    if removed_nodes_list:
        parts.append(
            f"{len(removed_nodes_list)} node"
            f"{'s' if len(removed_nodes_list) != 1 else ''} removed"
        )
    if removed_edges_list:
        parts.append(
            f"{len(removed_edges_list)} edge"
            f"{'s' if len(removed_edges_list) != 1 else ''} removed"
        )
